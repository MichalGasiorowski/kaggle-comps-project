...
class DataPath:
    def __init__(self, _data_root: str, competition: str):
        self._data_root = _data_root
        self._competition = competition
        self._competition_data_root = os.path.join(_data_root, competition)

        self._data_root_external = os.path.join(self._competition_data_root, 'external')
        self._data_root_interim = os.path.join(self._competition_data_root, 'interim')
        self._data_root_raw = os.path.join(self._competition_data_root, 'raw')
        self._data_root_processed = os.path.join(self._competition_data_root, 'processed')
    ...
    @property
    def competition_data_root(self):
        return self._competition_data_root
    ...
    def get_train_file_path(self, dir_root=external_train_dirpath):
        return f'{dir_root}/train.csv'

    def get_valid_file_path(self, dir_root):
        return f'{dir_root}/valid.csv'

    def get_test_file_path(self, dir_root=external_test_dirpath):
        return f'{dir_root}/test.csv'


def get_datapath(_data_root, competition):
    return DataPath(_data_root=_data_root, competition=competition)
...
def download_and_copy_data(data_path: DataPath, kaggle_competition: str, force: bool = False):
    ...
    #os.system(f'cp {data_path.data_root_external}/sample_submission.csv {data_path.data_root_external}')

    # clean up
    #os.system(f'rm {data_path.data_root_external}/*.csv {data_path.data_root_external}/*.zip')


def run(data_root: str, kaggle_competition: str, force=False):
    data_path = get_datapath(data_root, competition=kaggle_competition)
    data_path.make_dirs()

    download_and_copy_data(data_path=data_path, kaggle_competition=kaggle_competition, force=force)
    return data_path
